...
image::mausoleum.jpg[{image-80-width}]
...
image::beers.jpg[{image-60-width}]
...
\Huge
\begin{array}
&E(L) = \sum\limits_{n=0}^{\infty}n p_n = \rho(1-\rho)\sum\limits_{n=0}^{\infty}n \rho_{n-1}=\\ 
= \frac{\rho}{1- \rho}.
\end{array}
++++
...
&mu; >> &lambda; (очереди нет, но и процессор ничего не делает) 
